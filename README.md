...
default. Each is printed with `slp1_format` and a `normalized_meter_reward`;
non-SLP1 IAST output is also shown after conversion to SLP1.
...
`slp1_format_rate`, `mean_normalized_meter_reward`,
`normalized_perfect_rate`, and `normalized_nonzero_rate`. The legacy training
reward is retained only for diagnosing the completed run because it assumed all
outputs were already SLP1. This evaluates meter only; semantic fidelity still
requires a separate human or semantic evaluation.
...
Non-ASCII IAST output receives `0.0`, closing the transliteration loophole found
